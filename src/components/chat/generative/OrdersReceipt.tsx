...
import { ordersReceiptSchema, filterChips } from "@/components/orders/orders-filter";
...
export function OrdersReceipt(props: Record<string, unknown>): ReactElement | null {
  ...
  if (data.kind === "order") {
    ...
    return (
      <Receipt title={`Ordine #${data.number}`} sub={sub} amount={data.totalGross} />
    );
  }
...
function Receipt({
  ...
  return (
    <div className="flex items-baseline justify-between gap-3 rounded-[var(--radius-card)] border border-[var(--color-line)] bg-[var(--color-paper-muted)] px-3 py-2">
