...
import { usePortalsPanel } from "@/components/portals/portals-panel-context";
import { useCloseMobileChat } from "@/components/shell/MobileChatOverlay";

// Ricevuta di quello che Livia ha fatto al pannello Portali. In chat NON va mai
// la lista (420px, log che scorre): la lista e' gia' a fianco, qui basta una
// riga che rimette il pannello com'era.
export function PortalsReceipt(props: Record<string, unknown>): ReactElement | null {
  const apply = usePortalsPanel();
  const closeSheet = useCloseMobileChat();
  const parsed = portalsReceiptSchema.safeParse(props);
  ...
  if (data.kind === "portal") {
    return (
      <Receipt
        ...
        sub={[data.city, data.statusLabel].filter(Boolean).join(" · ")}
        onClick={onClick}
      />
    );
  }
  ...
  return (
    <Receipt
      title="Lista portali aggiornata"
      sub={chips.length ? chips.join(" · ") : "tutti i portali"}
      onClick={onClick}
    />
  );
}
...
function Receipt({
  ...
  title: string;
  sub: string;
  /** Presente solo dentro il pannello Portali: la riga torna a essere un'azione. */
  onClick?: () => void;
}): ReactElement {
